...
function startLoginCheck() {
  ...
  checkInterval = setInterval(async () => {
    if (polling.value || !loginKey) return;
    polling.value = true;
...
<template>
  <NModal :show="show" preset="card" title="账号管理" class="w-760px max-w-[95vw]" @update:show="emit('update:show', $event)" @after-leave="closeModal">

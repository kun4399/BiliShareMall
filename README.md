...
HTTP 429 表示 B 站对账号、设备或出口 IP 启用了请求频率限制。程序会对同一账号的市集请求统一限速（最低间隔 12 秒并加入随机抖动），遇到 429 后按 1、2、4 分钟逐级冷却并自动继续，最长自动冷却 15 分钟。请不要反复停止并立即重启任务，这会延长服务端的限流窗口。

网页登录态与程序保存的登录态相互独立。请在程序的账号管理中完成扫码登录并为任务绑定账号；程序会持久化并复用该账号会话。
